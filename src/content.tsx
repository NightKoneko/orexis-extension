...
import {
  ...
  POST_DOM_SETTLE_DELAY,
  MENU_SELECTORS,
  CONTENT_SELECTORS,
  SUBMENU_SELECTOR,
} from './constants'
import { OptimizerEquip } from './components/OptimizerEquip'
...
function waitForElement(selectors: string | readonly string[], timeout = ELEMENT_WAIT_TIMEOUT): Promise<Element> {
...
function injectTabContent(container: HTMLDivElement) {
  let tabsContainer = queryFirst(CONTENT_SELECTORS)
  if (!tabsContainer) {
    console.warn('[Orexis] Tabs container not found')
    return false
  }

  if (tabsContainer.id && tabsContainer.id !== OREXIS_PAGE_KEY && tabsContainer.parentElement) {
...
function injectOptimizerEquip() {
  const optimizerDiv = document.getElementById('OPTIMIZER')
  if (!optimizerDiv) {
    console.warn('[Orexis] Optimizer tab root div not found')
    return false
  }

  const sidebarDiv = optimizerDiv.childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[0]
  if (!sidebarDiv) {
    console.error("[Orexis] Failed to resolve sidebar")
    return false
  }
...
async function injectOrexis() {
  try {
    ...
    await new Promise(r => setTimeout(r, POST_DOM_SETTLE_DELAY))
    ...
    if(!injectOptimizerEquip()) {
      console.error("[Orexis] Failed to inject optimizer equip button")
      return
    }
    ...
    console.error('[Orexis] Failed to inject:', error)
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', injectOrexis)
} else {
  injectOrexis()
}
